...
- [ ] `docs/requirements.md`, `docs/usecases.md`, SSD, SD, class diagram이 동일한 유스케이스와 요구사항을 가리킨다.
- [ ] 전원 ON 중 기본 상태가 일반 모드 청소로 유지된다.
- [ ] 먼지와 장애물이 동시에 감지되면 먼지 대응 절차가 먼저 실행된다.
- [ ] 전진 중 먼지 감지 시 Boost + 시계 90도 회전 + 후방 확인 + 후진 전환이 수행된다.
- [ ] 후진 중 먼지 감지 시 Boost + 반시계 90도 회전 + 전방 확인 + 전진 전환이 수행된다.
- [ ] 전진 중 장애물 감지 시 일반 모드 + 시계 90도 회전 + 후방 확인 + 후진 전환이 수행된다.
- [ ] 후진 중 장애물 감지 시 일반 모드 + 반시계 90도 회전 + 전방 확인 + 전진 전환이 수행된다.
- [ ] 대상 센서가 막혀 있으면 같은 방향 90도 회전과 센서 재확인이 반복된다.
- [ ] 상태 조회에서 진행 방향, 회전 상태, 청소 출력 모드를 확인할 수 있다.
- [ ] 단위 테스트와 시스템 테스트가 통과한다.
