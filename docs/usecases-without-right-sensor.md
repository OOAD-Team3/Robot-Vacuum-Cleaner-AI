# RVC SW Controller Use Cases - Without Right Sensor
...
본 문서는 우측 장애물 센서가 제거된 구성을 기준으로 한다. 우측 감지가 필요한 경우 RVC는 우측으로 90도 회전한 뒤 전방 센서로 기존 우측 방향의 장애물 여부를 확인한다. 기존 우측 방향이 이동 가능하고 우측 회피를 수행할 수 있으면 탐색 회전 자세를 유지한 채 바로 진행하며, 기존 우측 방향이 막혀 있거나 좌측 회피 판단으로 돌아가야 할 때만 좌측으로 90도 회전해 원래 정면 방향으로 복귀한다.

## 2. Actors

| Actor | Description |
| --- | --- |
| 센서 입력 | 전방, 후방, 좌측 장애물 감지와 먼지 감지 상태를 SW Controller에 제공하는 추상화된 입력이다. 우측 장애물은 별도 센서 입력으로 직접 제공되지 않으며, 필요 시 우측 90도 회전 후 전방 센서로 확인한다. 후진 회피 흐름에서는 필요 시 후방 센서를 이용해 기존 우측 방향 장애물 여부를 판단한다. |
...
| UC-003 | 좌측 센서와 우측 90도 회전 탐색으로 회피 방향 선택 | 센서 입력 | FR-005, FR-008, NFR-006 |
| UC-004 | 방향 전환 후 청소 재개 | 센서 입력 | FR-006, FR-009 |
| UC-005 | 전방·좌측·우측 방향이 모두 막힌 경우 후진 | 센서 입력 | FR-007, FR-013 |
...
| Alternative/Exception Scenarios | A1. 전방 장애물 감지 후 좌측 회피가 가능하면 UC-003에서 좌측 회피를 우선 선택한다.<br>A2. 전방 장애물과 좌측 장애물이 감지되고, UC-003의 우측 90도 회전 탐색 결과 기존 우측 방향도 막힌 것으로 확인되면 UC-005로 전환한다.<br>A3. 정지 중 먼지가 감지되어도 장애물 회피 판단을 우선 수행하고, 청소 출력 상태는 UC-006에 따라 별도로 처리한다. |
| Result | RVC는 전방 장애물 앞에서 정지하고 회피 판단을 준비한다. |

### UC-003. 좌측 센서와 우측 90도 회전 탐색으로 회피 방향 선택

| Item | Description |
| --- | --- |
| Preconditions | RVC가 장애물 회피를 위해 방향 전환이 필요한 상태이다. |
| Main Scenario | 1. 좌측 장애물 감지 상태를 확인한다.<br>2. 좌측이 이동 가능하면 좌측 회피 방향을 우선 선택한다.<br>3. 좌측이 막혀 있어 우측 이동 가능 여부 확인이 필요하면 주행 장치에 우측으로 90도 회전 명령을 전달한다.<br>4. 회전 후 전방 센서로 기존 우측 방향의 장애물 여부를 확인한다.<br>5. 기존 우측 방향이 이동 가능하면 원래 정면 방향으로 복귀하지 않고 현재 자세를 우측 회피 완료 자세로 사용한다.<br>6. 기존 우측 방향이 막혀 있으면 주행 장치에 좌측으로 90도 회전 명령을 전달해 원래 정면 방향으로 복귀한다.<br>7. 좌측과 기존 우측 방향이 모두 막혀 있으면 UC-005로 전환한다.<br>8. 선택된 좌측 회피 방향 또는 현재 우측 회피 완료 자세를 청소 재개 단계로 전달한다. |
| Alternative/Exception Scenarios | A1. 좌측이 이동 가능하면 우측 탐색을 수행하지 않고 좌측 회피 방향을 선택한다.<br>A2. 좌측이 막혀 있고 우측 탐색 결과가 이동 가능하면 복귀하지 않고 현재 자세로 UC-004에 전환한다.<br>E1. 좌측이 막혀 있고 우측 탐색 결과도 막혀 있으면 원래 정면 방향으로 복귀한 뒤 UC-005로 전환한다. |
...
| Item | Description |
| --- | --- |
| Preconditions | RVC가 장애물 회피를 위한 방향 전환을 완료했거나, 우측 탐색을 위한 90도 회전 자세를 그대로 우측 회피 완료 자세로 사용하기로 결정했다. |
...
| Alternative/Exception Scenarios | A1. 방향 전환 후 전방 장애물이 다시 감지되면 UC-003을 다시 수행한다.<br>A2. 먼지 대응 상태가 유지 중이면 일반 출력으로 덮어쓰지 않고 증가 출력 상태를 유지한다.<br>A3. 우측 탐색을 위해 우측으로 90도 회전했고 기존 우측 방향이 이동 가능하면 탐색 회전 자세를 유지한 채 청소를 재개한다.<br>A4. 우측 탐색 결과가 막혀 있으면 원래 정면 방향으로 복귀한 뒤 UC-005 또는 추가 회피 판단 흐름으로 전환한다. |
| Result | RVC는 회피 후 다시 전진하며 청소를 계속한다. |

### UC-005. 전방·좌측·우측 방향이 모두 막힌 경우 후진

| Item | Description |
| --- | --- |
| Preconditions | 전방 장애물이 감지되어 있고, 좌측 장애물이 감지되어 있으며, UC-003의 우측 90도 회전 탐색 또는 후진 회피 중 후방 센서 판단을 통해 기존 우측 방향도 막힌 것으로 확인되어 있다. |
| Main Scenario | 1. SW Controller는 전방, 좌측, 우측 방향이 모두 막힌 상태와 후방 장애물 상태를 확인한다.<br>2. 주행 장치에 정지 명령을 전달한다.<br>3. 후방 장애물이 없어 후진할 수 있으면 주행 장치에 후진 명령을 전달한다.<br>4. 후진 동작이 끝난 뒤 좌측 장애물 상태를 먼저 확인한다.<br>5. 후진 후 좌측이 이동 가능하면 우측 탐색을 수행하지 않고 좌측 회피 방향을 우선 선택한다.<br>6. 후진 후 좌측이 막혀 있어 우측 방향 확인이 필요하면 UC-003과 동일하게 우측으로 90도 회전한 뒤 전방 센서로 기존 우측 방향 장애물 여부를 확인한다.<br>7. 후진 후 우측 탐색 결과가 이동 가능하면 원래 정면 방향으로 복귀하지 않고 현재 자세를 우측 회피 완료 자세로 사용해 UC-004로 전환한다.<br>8. 후진 후 우측 탐색 결과도 막혀 있으면 좌측으로 90도 회전해 원래 정면 방향으로 복귀한 뒤 정지 또는 추가 후진 판단 상태를 유지한다. |
| Alternative/Exception Scenarios | A1. 후진 후 좌측이 이동 가능하면 UC-003으로 전환해 좌측 회피 방향을 우선 선택한다.<br>A2. 후진 후 좌측이 막혀 있고 우측 탐색 결과가 이동 가능하면 복귀하지 않고 현재 자세로 UC-004에 전환한다.<br>A3. 후진 후 좌측이 막혀 있고 후방 센서 판단만으로 기존 우측 방향이 이동 가능하다고 판단되면 선택된 우측 방향 전환을 수행한다.<br>E1. 후방 장애물이 감지되어 더 이상 후진할 수 없으면 정지 상태를 유지한다.<br>E2. 후진 후에도 좌측과 우측 방향이 모두 막혀 있으면 원래 정면 방향으로 복귀한 뒤 정지 또는 추가 후진 판단 상태를 유지한다. |
...
- 본 문서에서 우측 감지는 우측 센서 입력을 의미하지 않는다. 우측 감지가 필요한 경우 RVC는 우측으로 90도 회전하고 전방 센서로 기존 우측 방향을 확인한다. 기존 우측 방향이 이동 가능하고 우측 회피를 수행할 수 있으면 복귀하지 않고 현재 자세로 진행하며, 기존 우측 방향이 막혀 있거나 좌측 회피 판단으로 돌아가야 할 때만 좌측으로 90도 회전해 원래 정면 방향으로 복귀한다.
- 후진 회피 흐름에서는 로봇의 자세와 후진 경로에 따라 후방 센서를 기존 우측 방향 확인에 사용할 수 있다.
