...
| Main Scenario | 1. SW Controller는 삼면 장애물 상태를 확인한다.<br>2. 주행 장치에 정지 명령을 전달한다.<br>3. 후진할 수 있으면 주행 장치에 후진 명령을 전달한다.<br>4. 후진 동작이 끝난 뒤 방향 전환 판단 단계로 상태를 넘긴다. |
| Alternative/Exception Scenarios | A1. 후진 후 좌측만 이동 가능하면 UC-003으로 전환해 좌측 회피 방향을 선택한다.<br>A2. 후진 후 우측만 이동 가능하면 UC-003으로 전환해 우측 회피 방향을 선택한다.<br>A3. 후진 후 좌측과 우측이 모두 이동 가능하면 UC-003으로 전환해 정책에 따라 회피 방향을 선택한다.<br>E1. 더 이상 후진할 수 없는 상태에서도 좌측과 우측이 모두 막혀 있으면 정지 상태를 유지한다. |
